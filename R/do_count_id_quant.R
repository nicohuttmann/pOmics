...
#'
#' @param data_ data_
#' @param input.id dataframe of identified proteins
#' @param input.quant dataframe of quanitfied proteins
...
#'
#'
do_count_id_quant <- function(data_, input.id = "Peptides", input.quant = "LFQ.intensity", output = "data_count_id_quant") {
  ...
  data_[[output]] <- data_[[input.id]] %>%
    ...
    dplyr::inner_join(data_[[input.quant]] %>%
                        dplyr::rowwise() %>%
