...
add_segment_pvalue <- function(segment, dend.enrich) {
  ...
  # Add l for y lines (inherited from previous cluster)
  segment[(segment$x == x) %and% (segment$xend != x), "l"] <- 2:1

  xlines <- segment[(segment$x == x) %and% (segment$y == segment$yend), "y"]
  ...
  if (n > 2) {
    ...
    segment[(segment$x == x) %and% (segment$xend != x), "l"] <- rank(-c(segment[(segment$x == x) %and% (segment$xend != x), "y"],
                                                                        setdiff(xlines, segment[(segment$x == x) %and% (segment$xend == x), "y"])))[1:2]


    xlines <- c(setdiff(xlines, segment[(segment$x == x) %and% (segment$xend == x), "y"]),
                segment[(segment$x == x) %and% (segment$xend != x), "y"])
