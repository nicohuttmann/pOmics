...
#'
#' 
setup_annotations <- function(taxId, OrgDb, dataset) {
  
  # Taxonomy Id
  if (!hasArg(taxId)) {
    taxId <- identify_taxId(get_variables(dataset))
  }
  
  # Species name
  scientific_name <- UniProt.ws::lookupUniprotSpeciesFromTaxId(taxId)
