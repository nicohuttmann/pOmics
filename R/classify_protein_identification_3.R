...
#'
#'
classify_protein_identification_3 <- function(data.eval, lower.limit = 0, dataset, plot = T, save = F) {

  # Check data
  if (ncol(data.eval) != 4 || colnames(data.eval)[1] != "variables" || mode(data.eval[[2]]) != "numeric")
  ...
  if (save) {
    ...
    for (i in results.list[["combinations"]]) {
